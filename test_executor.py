...
class CreateJobs:
    def __init__(self, jobs, job_name):
        ...
        self.browser_time, self.diff, self.start_time, self.time = ab_test_execute_times(self.job)
        self.schedule = None
        self.total_minutes_in_month = 30 * 24 * 60
        self.total_minutes_in_2_weeks = 15 * 24 * 60

    def jobs(self):
        self.api_info = self.api_infos['ab_test']
        self.url = get_api_url(host=self.api_info['host'],
                               port=self.api_info['port'],
                               api_name=self.api_info['api_name'])
        request_url(self.url, self.api_info['execute']['params'])
        print("requests are sent!!!")

    def stop_job(self, request=True):
        if self.job['active'] is True:  # if there is active job update ab_test_execute.yaml
            self.logger.regenerate_file()
            self.jobs_yaml[self.job_name]['active'] = False
            write_yaml(conf('docs_main_path'), "ab_test_execute.yaml", self.jobs_yaml)
            self.api_info = self.api_infos[self.api_infos['ab_test']]
            self.url = get_api_url(host=self.api_info['host'],
                                   port=self.api_info['port'],
                                   api_name=self.api_info['api_name'])
            if request:
...
def start_job(job):
    Logger('ab_test_execute_' + job)
    print("received :", {'job': job}, " time :", get_time())
    j = CreateJobs(read_yaml(conf('docs_main_path'), 'ab_test_execute.yaml'), job)
    if j.job['day'] == 'only once':
        j.job_that_executes_once()
        return 'done!!!!'
    if j.job['day'] in ['Monthly', 'Every 2 Weeks']:
        while True:
            jobs = read_yaml(conf('docs_main_path'), 'ab_test_execute.yaml')
...
def start_job_and_update_job_active(jobs, job):
    ...
    write_yaml(conf('docs_main_path'), "ab_test_execute.yaml", jobs)
    ab_test_execute_api = read_yaml(conf('docs_main_path'), 'apis.yaml')['ab_test_execute']
    url = get_api_url(ab_test_execute_api['host'], ab_test_execute_api['port'], ab_test_execute_api['api_name'])
    request_url(url, {'job': job})


if __name__ == '__main__':
    api = read_yaml(conf('docs_main_path'), 'apis.yaml')['ab_test_execute']
    api_executor(api)
